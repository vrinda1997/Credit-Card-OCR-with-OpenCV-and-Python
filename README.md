...
## load the reference OCR-A image from disk, convert it to grayscale and threshold it, such that the digits appear as *white* on a *black* background
# and invert it, such that the digits appear as *white* on a *black*
ref = cv2.imread(args["reference"])
ref = cv2.cvtColor(ref, cv2.COLOR_BGR2GRAY)
ref = cv2.threshold(ref, 10, 255, cv2.THRESH_BINARY_INV)[1]
## find contours in the OCR-A image (i.e,. the outlines of the digits and sort them from left to right, and initialize a dictionary to map digit name to the ROI
...
## apply a closing operation using the rectangular kernel to help close gaps in between credit card number digits, then apply Otsu's thresholding method to binarize the image
